...
import { Input, Modal, Select, Table,DatePicker, Form, message } from "antd";
...
const HomePage = () => {
  ...
  useEffect(() => {
    const getAllTransaction = async (req,res) =>{
      ...
      console.log(error);
      message.error('Fetch Issue with Trensaction');
      }
    };
    ...
    <Layout>
      ...
      <div className="filters">
        ...
        <div className="switch-icons">
              <UnorderedListOutlined className={`mx-2 ${viewData === 'table' ? 'ative-icon' : 'inactive-icon'}`}
               onClick={() => setViewData('table')} />
              <AreaChartOutlined className={`mx-2 ${viewData === 'analytics' ? 'ative-icon' : 'inactive-icon'}`}
